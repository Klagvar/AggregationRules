...
/**
 * Функция весов позиций phi(k) для позиционно-взвешенной медианы Кемени.
...
public interface PositionWeightFunction {
    ...
    /**
     * Гиперболическая функция: phi(k) = 1/k.
    ...
    /**
     * Линейная функция: phi(k) = (m - k + 1) / m.
    ...
    /**
     * Экспоненциальная функция: phi(k) = e^(-α(k-1)).
    ...
    /**
     * Top-K функция: phi(k) = 1 если k ≤ K, иначе 0.
    ...
    /**
     * Логарифмическая функция: phi(k) = 1 / log₂(k + 1).
     * Мягкое затухание.
